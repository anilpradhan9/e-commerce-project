            ...
            return <div key={index} className='bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] -z-10'>
              <div className='flex flex-col md:flex-row gap-10 justify-center h-[600px] my-20 md:my-0 items-center px-4'>
                <div className='md:space-y-6 space-y-3'>
                  <h3 className='text-red-500 font-semibold font-sans text-sm'>Powering Your World With the Best in element</h3>
                  <h1 className='text-xl md:text-4xl font-bold uppercase md:line-clamp-3 line-clamp-2 md:w-[500px] text-white'>{item.title}</h1>
                ...
                <div>
                  <img src={item.image} alt={item.title} className='rounded-full w-[550px] hover:scale-10s transition-all shadow-2xl shadow-red-400' />
                </div>
              </div>
            </div>
          })
